...
char i2c_reg8_read(XIicPs *InstancePtr, char IIC_ADDR, s32 ByteCount, char Addr)
{
	u8 wr_data, rd_data;

	wr_data = Addr;
	XIicPs_MasterSendPolled(InstancePtr, &wr_data, ByteCount, IIC_ADDR);
	XIicPs_MasterRecvPolled(InstancePtr, &rd_data, ByteCount, IIC_ADDR);
	while (XIicPs_BusIsBusy(InstancePtr));
	return rd_data;
}
