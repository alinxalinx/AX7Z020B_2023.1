...
int main()
{
	...
	while(1)
	{
		if (key_flag)
		{
			 ...
			 * Write led value
			 */
			XGpioPs_WritePin(&GPIO_PTR, 54, LedVal) ;
			LedVal = ~LedVal ;
			/*
...
int PsGpioSetup(XScuGic *InstancePtr)
{
	...
	/* set MIO 55 as input */
	XGpioPs_SetDirectionPin(&GPIO_PTR, 55, GPIO_INPUT) ;
	/* set MIO 54 as output */
	XGpioPs_SetDirectionPin(&GPIO_PTR, 54, GPIO_OUTPUT) ;
	/* enable MIO 54 output */
	XGpioPs_SetOutputEnablePin(&GPIO_PTR, 54, GPIO_OUTPUT) ;
	/* set interrupt type */
	XGpioPs_SetIntrTypePin(&GPIO_PTR, 55, XGPIOPS_IRQ_TYPE_EDGE_RISING) ;

	/* enable GPIO interrupt */
	XGpioPs_IntrEnablePin(&GPIO_PTR, 55) ;
