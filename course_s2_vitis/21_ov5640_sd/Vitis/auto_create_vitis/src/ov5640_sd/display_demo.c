...
int main(void)
{
	...
	/* Set PS LED off */
	XGpioPs_WritePin(&GpioInstance, 55, 1) ;
	...
	while(1)
	{
		if (key_flag == 2)
		{
			...
			/* Set PS LED on */
			XGpioPs_WritePin(&GpioInstance, 55, 0) ;
...
int GpioSetup(XScuGic *InstancePtr, u16 DeviceId, u16 IntrID, XGpioPs *GpioInstancePtr)
{
	...
	/* set MIO 54 as input */
	XGpioPs_SetDirectionPin(GpioInstancePtr, 54, GPIO_INPUT) ;
	/* set interrupt type */
	XGpioPs_SetIntrTypePin(GpioInstancePtr, 54, XGPIOPS_IRQ_TYPE_EDGE_RISING) ;

	/* set MIO 0 as output */
	XGpioPs_SetDirectionPin(&GpioInstance, 55, GPIO_OUTPUT) ;
	/* enable MIO 0 output */
	XGpioPs_SetOutputEnablePin(&GpioInstance, 55, GPIO_OUTPUT) ;
	...
	XGpioPs_IntrEnablePin(GpioInstancePtr, 54) ;
...
void GpioHandler(void *CallbackRef)
{
	...
	IntVal = XGpioPs_IntrGetStatusPin(GpioInstancePtr, 54) ;
	/* clear key interrupt */
	XGpioPs_IntrClearPin(GpioInstancePtr, 54) ;
